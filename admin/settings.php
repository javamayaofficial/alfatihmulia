...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'profil') {
        ...
        flash_set('Pengaturan laporan publik disimpan.');
    } elseif ($act === 'payments') {
        foreach (['payment_bank_primary_name', 'payment_bank_primary_number', 'payment_bank_primary_holder'] as $k) {
            if (isset($_POST[$k])) set_setting($k, trim($_POST[$k]));
        }
        $upload = upload_public_file('payment_qris_upload', 'payment-qris', ['jpg', 'jpeg', 'png', 'webp']);
        if (!$upload['ok']) {
            flash_set($upload['message'], 'err');
            header('Location: '.admin_url('settings')); exit;
        }
        ...
        flash_set('Pengaturan pembayaran manual disimpan.');
        ...
        if ($id > 0) {
            ...
            flash_set('Rekening tambahan diperbarui.');
        } else {
            ...
            flash_set('Rekening tambahan ditambahkan.');
        }
        ...
        if ($existingBank) {
            ...
            flash_set('Rekening tambahan dihapus.');
        }
        ...
        if ($currentUrl !== '') {
            ...
            flash_set('File QRIS pembayaran dihapus.');
        } else {
        ...
        if ($currentUrl !== '') {
            ...
            flash_set($allDocKeys[$docKey] . ' berhasil dihapus.');
        } else {
...
$renderSecretInput = function ($name, $value, $placeholder = '') {
    ...
    <div class="secret-input-row">
      <div class="secret-input-stack">
        <input type="password" id="<?= e($inputId) ?>" name="<?= e($name) ?>" value="<?= e($value) ?>" placeholder="<?= e($placeholder) ?>" autocomplete="off" data-secret-input>
...
<div class="panel">
  ...
  <form method="post" class="form" enctype="multipart/form-data"><?= csrf_field() ?><input type="hidden" name="act" value="profil">
    <div class="grid-2">
    ...
    <div class="grid-2">
      <div>
        <label>Upload Dokumen Akta</label><input type="file" name="legal_akta_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
      ...
      <div>
        <label>Upload Dokumen SK</label><input type="file" name="legal_sk_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
      ...
      <div>
        <label>Upload Dokumen NPWP</label><input type="file" name="legal_npwp_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
...
<div class="panel">
  ...
  <form method="post" class="form"><?= csrf_field() ?><input type="hidden" name="act" value="api">
    ...
    <?= $renderSecretInput('duitku_merchant_code', setting('duitku_merchant_code'), 'Merchant Code project Duitku') ?>
    <label>Duitku API Key</label>
    <?= $renderSecretInput('duitku_api_key', setting('duitku_api_key'), 'API Key project Duitku') ?>
    <label class="check"><input type="checkbox" name="duitku_is_production" value="1" <?= setting('duitku_is_production','0') === '1' ? 'checked' : '' ?>> Gunakan mode production Duitku</label>
    <div class="hero-cta">
      <button class="btn" type="submit" name="test_provider" value="duitku">Test Koneksi Duitku</button>
    </div>
    ...
    <?= $renderSecretInput('mailketing_api_token', setting('mailketing_api_token'), 'API token dari menu integration Mailketing') ?>
    ...
    <?= $renderSecretInput('fonnte_token', setting('fonnte_token'), 'Notifikasi WA otomatis & OTP') ?>
...
<div class="panel">
  ...
  <form method="post" class="form" enctype="multipart/form-data"><?= csrf_field() ?><input type="hidden" name="act" value="payments">
    ...
    <div class="grid-2">
      <div>
        <label>Upload QRIS</label><input type="file" name="payment_qris_upload" accept=".jpg,.jpeg,.png,.webp">
    ...
    <button class="btn btn-primary">Simpan Pembayaran Manual</button>
  </form>
...
<div class="panel">
  ...
  <form method="post" class="form" enctype="multipart/form-data"><?= csrf_field() ?><input type="hidden" name="act" value="laporan">
    <div class="grid-2">
      <div>
        <label>Laporan Bulanan</label><input type="text" name="laporan_doc_bulanan" value="<?= e(setting('laporan_doc_bulanan')) ?>" placeholder="https://...">
        <input type="file" name="laporan_doc_bulanan_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
      ...
      <div>
        <label>Laporan Tahunan</label><input type="text" name="laporan_doc_tahunan" value="<?= e(setting('laporan_doc_tahunan')) ?>" placeholder="https://...">
        <input type="file" name="laporan_doc_tahunan_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
      ...
      <div>
        <label>Laporan Penyaluran Program</label><input type="text" name="laporan_doc_penyaluran_program" value="<?= e(setting('laporan_doc_penyaluran_program')) ?>" placeholder="https://...">
        <input type="file" name="laporan_doc_penyaluran_program_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
      ...
      <div>
        <label>Audit Keuangan</label><input type="text" name="laporan_doc_audit_keuangan" value="<?= e(setting('laporan_doc_audit_keuangan')) ?>" placeholder="https://...">
        <input type="file" name="laporan_doc_audit_keuangan_file" accept=".pdf,.jpg,.jpeg,.png,.webp">
    ...
    <label class="check"><input type="checkbox" name="laporan_show_donors" value="1" <?= setting('laporan_show_donors','0') === '1' ? 'checked' : '' ?>> Tampilkan daftar donatur publik secara terbatas pada halaman laporan</label>
    <button class="btn btn-primary">Simpan Pengaturan Laporan</button>
  </form>
</div>
