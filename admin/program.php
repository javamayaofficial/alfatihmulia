...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save') {
        ...
        if ($title !== '') {
            if ($id) {
                DB::run("UPDATE programs SET title=?,category=?,excerpt=?,description=?,target_amount=?,beneficiaries=?,video_url=?,status=? WHERE id=?",
                  'ssssiissi',[$title,$cat,$excerpt,$desc,$target,$benef,$video?:null,$status,$id]);
                audit('update_program', $title);
                flash_set('Program diperbarui.');
            } else {
                $slug = slugify($title);
                DB::run("INSERT INTO programs (slug,title,category,excerpt,description,target_amount,beneficiaries,video_url,status) VALUES (?,?,?,?,?,?,?,?,?)",
                  'sssssiiss',[$slug,$title,$cat,$excerpt,$desc,$target,$benef,$video?:null,$status]);
...
<div class="grid-2">
  <div class="panel">
    ...
    <p class="note">Catatan: upload gambar program via File Manager ke <code>assets/img/</code> lalu set nama file di database (opsional pada versi ini).</p>
  </div>
  ...
  <div class="panel">
    ...
    <div class="table-wrap"><table class="table">
      ...
      <tbody><?php foreach ($rows as $p): ?>
        <tr><td><b><?= e($p['title']) ?></b><br><small class="muted"><?= e($p['category']) ?></small></td>
